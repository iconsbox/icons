/**
 * THIS IS AN AUTO GENERATED FILE, CHANGES WILL NOT APPLY
 */
import * as React from "react";
import PropTypes from "prop-types";

const TargetIcon = ({ className, size }) => (
  <svg
    data-testid="TargetIcon"
    viewBox="0 0 100 100"
    className={className}
    style={{
      width: size * 10,
      height: size * 10
    }}
    focusable="false"
    fill="currentColor"
  >
    <g>
      <path
        d="M49.984,27.452c-12.453,0-22.548,10.095-22.548,22.548c0,12.453,10.095,22.548,22.548,22.548S72.531,62.453,72.531,50
		C72.531,37.547,62.436,27.452,49.984,27.452z M49.984,62.608c-6.964,0-12.609-5.645-12.609-12.608
		c0-6.964,5.645-12.609,12.609-12.609S62.592,43.036,62.592,50C62.592,56.963,56.947,62.608,49.984,62.608z"
      ></path>
      <path
        d="M50,12.5c-20.712,0-37.5,16.793-37.5,37.502C12.5,70.712,29.288,87.5,50,87.5c20.712,0,37.5-16.788,37.5-37.498
		C87.5,29.293,70.712,12.5,50,12.5z M50,77.561c-15.199,0-27.56-12.362-27.56-27.559c0-15.195,12.362-27.562,27.56-27.562
		c15.198,0,27.56,12.367,27.56,27.562C77.56,65.198,65.198,77.561,50,77.561z"
      ></path>
      <circle cx="49.984" cy="50" r="7.621"></circle>
    </g>
  </svg>
);

TargetIcon.propTypes = {
  className: PropTypes.string,
  size: PropTypes.number
};

TargetIcon.defaultProps = {
  size: 1.5
};

export default TargetIcon;
