/**
 * THIS IS AN AUTO GENERATED FILE, CHANGES WILL NOT APPLY
 */
import * as React from "react";
import PropTypes from "prop-types";

const FootIcon = ({ className, size }) => (
  <svg
    data-testid="FootIcon"
    viewBox="0 0 100 100"
    className={className}
    style={{
      width: size * 10,
      height: size * 10
    }}
    focusable="false"
    fill="currentColor"
  >
    <g>
      <circle cx="28.549" cy="35.885" r="4.342"></circle>
      <circle cx="65.794" cy="15.531" r="8.767"></circle>
      <circle cx="47.712" cy="18.329" r="5.469"></circle>
      <path
        d="M72.903,74.043c-1.387-2.237-2.202-4.867-2.202-7.693c0-2.464,0.616-4.782,1.692-6.82l-0.034-0.051
		c2.168-3.252,3.436-7.155,3.436-11.356c0-11.326-9.182-20.507-20.507-20.507c-11.326,0-20.507,9.181-20.507,20.507
		c0,2.212,0.36,4.338,1.009,6.335c0.436,1.792,1.139,3.475,2.068,5.011l-0.096,0.055l14.854,25.729
		c1.568,4.64,5.947,7.984,11.115,7.984c6.484,0,11.743-5.256,11.743-11.741c0-2.813-0.992-5.394-2.643-7.416L72.903,74.043z"
      ></path>
      <circle cx="36.161" cy="26.693" r="4.342"></circle>
    </g>
  </svg>
);

FootIcon.propTypes = {
  className: PropTypes.string,
  size: PropTypes.number
};

FootIcon.defaultProps = {
  size: 1.5
};

export default FootIcon;
