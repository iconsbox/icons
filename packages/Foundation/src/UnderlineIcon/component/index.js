/**
 * THIS IS AN AUTO GENERATED FILE, CHANGES WILL NOT APPLY
 */
import * as React from "react";
import PropTypes from "prop-types";

const UnderlineIcon = ({ className, size }) => (
  <svg
    data-testid="UnderlineIcon"
    viewBox="0 0 100 100"
    className={className}
    style={{
      width: size * 10,
      height: size * 10
    }}
    focusable="false"
    fill="currentColor"
  >
    <g>
      <path
        d="M77.5,75.545c-0.036,0-0.068,0.009-0.103,0.01v-0.01h-55v0.01c-1.608,0.056-2.897,1.368-2.897,2.99s1.288,2.934,2.897,2.99
		v0.01h55v-0.01c0.035,0.001,0.068,0.01,0.103,0.01c1.656,0,3-1.343,3-3S79.156,75.545,77.5,75.545z"
      ></path>
      <path
        d="M50,72.12c15.829,0,23.581-9.057,23.581-22.521V21.383c0-1.617-1.311-2.928-2.929-2.928h-3.864
		c-1.618,0-2.929,1.311-2.929,2.928c0,0.04,0.01,0.076,0.012,0.116v27.856c0,8.649-4.814,14.28-13.871,14.28
		c-9.057,0-13.871-5.631-13.871-14.28V21.49c0.001-0.036,0.011-0.071,0.011-0.107c0-1.617-1.311-2.928-2.928-2.928h-3.865
		c-1.617,0-2.929,1.311-2.929,2.928v28.216C26.418,63.063,34.252,72.12,50,72.12z"
      ></path>
    </g>
  </svg>
);

UnderlineIcon.propTypes = {
  className: PropTypes.string,
  size: PropTypes.number
};

UnderlineIcon.defaultProps = {
  size: 1.5
};

export default UnderlineIcon;
