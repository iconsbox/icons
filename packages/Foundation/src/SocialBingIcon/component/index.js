/**
 * THIS IS AN AUTO GENERATED FILE, CHANGES WILL NOT APPLY
 */
import * as React from "react";
import PropTypes from "prop-types";

const SocialBingIcon = ({ className, size }) => (
  <svg
    data-testid="SocialBingIcon"
    viewBox="0 0 100 100"
    className={className}
    style={{
      width: size * 10,
      height: size * 10
    }}
    focusable="false"
    fill="currentColor"
  >
    <g>
      <ellipse cx="50.156" cy="56.277" rx="19.383" ry="13.568"></ellipse>
      <path
        d="M77.132,38.339c-7.063-4.908-15.964-7.331-24.577-7.779c-10.167-0.102-20.041,1.224-29.082,7.514v-19.34h-11.32v35.044
		c-0.129,14.675,9.747,20.961,22.465,25.78c4.621,1.14,9.741,1.709,15.36,1.709c5.526,0,10.588-0.569,15.186-1.709
		c4.597-1.141,8.568-2.973,11.912-4.984c3.344-2.012,5.944-4.365,7.802-7.061c1.857-2.694,2.786-5.572,2.786-8.63
		C89.094,48.444,81.907,41.657,77.132,38.339z M53.313,75.017c-12.718,0.398-27.624-3.441-29.07-18.361
		c-0.804-8.291,7.453-18.73,25.735-19.374c7.517-0.265,25.052,3.927,26.249,18.091C76.964,64.104,66.031,74.62,53.313,75.017z"
      ></path>
    </g>
  </svg>
);

SocialBingIcon.propTypes = {
  className: PropTypes.string,
  size: PropTypes.number
};

SocialBingIcon.defaultProps = {
  size: 1.5
};

export default SocialBingIcon;
